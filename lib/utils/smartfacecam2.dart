...
import 'package:Mem/utils/builders.dart';
import 'package:Mem/utils/enum_handler.dart';
import 'package:Mem/utils/face_camera.dart';
import 'package:Mem/utils/face_identifier.dart';
import 'package:Mem/utils/face_painter.dart';
import 'package:Mem/utils/hole_painter.dart';
import 'package:Mem/utils/logger.dart';
import 'package:camera/camera.dart';
import 'package:flutter/material.dart';
import 'package:iconsax/iconsax.dart';

class SmartFaceCamera2 extends StatefulWidget {
  ...
  final void Function(File? image) onFoodIntake;
  ...
  @override
  State<SmartFaceCamera2> createState() => _SmartFaceCamera2State();
}

class _SmartFaceCamera2State extends State<SmartFaceCamera2>
    with WidgetsBindingObserver, TickerProviderStateMixin {
  CameraController? _controller2;

  bool _alreadyCheckingImage = false;

  DetectedFace? _detectedFace2;
  ...
  Future<void> _initCamera() async {
    ...
    if (cameras.isNotEmpty) {
      _controller2 = CameraController(cameras.first,
      ...
      await _controller2!.initialize().then((_) {
        if (!mounted) {
          return;
        }
        setState(() {});
                        ...
                        if (_detectedFace2 != null) ...[
                          SizedBox(
                              ...
                              child: widget.indicatorBuilder?.call(
                                      ...
                                      _detectedFace2,
                                      Size(
                                        _controller2!.value.previewSize!.height,
                                        _controller2!.value.previewSize!.width,
                                      )) ??
                                  CustomPaint(
                                    painter: FacePainter(
                                        face: _detectedFace2!.face,
        ...
        if (widget.showControls) ...[
          Align(
            ...
            child: Padding(
              ...
              child: _captureControlWidget(size, widget.onFoodIntake),
            ),
          )
        ]
      ],
    ...
    if (cameraController != null && cameraController.value.isInitialized) {
      return CameraPreview(cameraController, child: Builder(builder: (context) {
        if (widget.messageBuilder != null) {
          return widget.messageBuilder!.call(context, _detectedFace2);
        }
  ...
  Widget _captureControlWidget(
    Size size,
    void Function(File? image)? onFoodIntake,
  ) {
    final CameraController? cameraController = _controller2;
    ...
    try {
      cameraController!.stopImageStream().whenComplete(() async {
        ...
        takePicture().then((XFile? file) {
          ...
          if (file != null) {
            widget.onFoodIntake(File(file.path));
          }
    ...
    if (!_alreadyCheckingImage && mounted) {
      ...
      try {
        await FaceIdentifier.scanImage(
            ...
            .then((result) async {
          setState(() => _detectedFace2 = result);
